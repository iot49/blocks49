...
* UI: A static webapp running typically on a smartphone that records the model railroad, saves views in ".r49" files, offers the user to pick marker locations, and runs a classifier at the marker locations. In live mode, the images are recorded continuously (at typically 0.5 to 10 fps) and runs the classifier at all marker locations. The ui is also used to collect training data for the classifier.

* Classifier: A fastai/Python framework to train the classifier and export it to the ui.
...
* Backend: A python app for controlling trains using the outputs from the ui. Communication between the ui and backend is with secure MQTT.
...
* lit/vite/typescript for the ui
